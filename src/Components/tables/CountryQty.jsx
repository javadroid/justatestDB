        ...
        <div>
          ...
          <ul className={`roll pt-2 width-full height-10 overflow-hidden scrollbar-thin scrollbar-thumb-color-bg_primary-500 max-h-96 overflow-y-auto`}>
            {data.filter((country) => {
              if (searchTerm == "") {
                return country;
              } else if (country.country_name.toLowerCase().includes(searchTerm.toLowerCase())) {
                return country;
              } else {
                return null;
              } 
            ...
            {
              searchTerm != "" && data.filter((country) => {
                return country.country_name.toLowerCase().includes(searchTerm.toLowerCase()).length === 0 && <li className="p-2 flex justify-between text-xs items-center md:text-lg">No results found</li>
              })
            }
          </ul>
